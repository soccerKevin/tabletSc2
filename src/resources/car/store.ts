...
interface CarStore extends CarState {
  ...
  removeFirstQueuePosition: (carId: number) => void;
...
export const useCarStore = create<CarStore>((set, get) => ({
  ...
  selectCar: (car, clearOthers = true) =>
    set((state) => {
  ...
  getNextQueuePosition: (carId) => {
    ...
    // Return the next position WITHOUT removing it from queue
    // CarManager will handle removing it when the car actually reaches it
    return car.moveQueue[0];
  },

  removeFirstQueuePosition: (carId) => {
    set((state) => ({
      cars: state.cars.map((c) =>
        c.id === carId ? { ...c, moveQueue: c.moveQueue.slice(1) } : c,
      ),
    }));
  },
  ...
  createCarIcon: (isSelected: boolean) => {
    ...
    const glowEffect = isSelected
      ? `<circle cx="${(size + 6) / 2}" cy="${(size + 6) / 2}" r="${size / 2 + 3}" fill="none" stroke="#00ff00" stroke-width="2" opacity="0.6"/>`
      : "";
